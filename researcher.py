...
async def research_agent(query: Query):
    try:
        ...
        results = agent_executor.stream({"input": query.query})
        final_output = ""
        ...
        if final_output:
            return {"status": "success", "data": final_output.strip()}
        else:
            return {"status": "no data", "message": "No relevant information found."}
